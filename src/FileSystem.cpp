...
{
    ESP_LOGI('FileSystem', "Listing directory: %s\n", dirname);
    ...
    if (!root.isDirectory())
    {
        ESP_LOGI('FileSystem', "Not a directory");
        return;
    }
...
void createDir(fs::FS &fs, const char *path)
{
    ...
    if (fs.mkdir(path))
        ESP_LOGI('FileSystem', "Dir created");
    else
        ESP_LOGE('FileSystem', "rmdir failed");
}
...
void removeDir(fs::FS &fs, const char *path)
{
    ...
    if (fs.rmdir(path))
        ESP_LOGI('FileSystem', "Dir removed");
    else
        ESP_LOGE('FileSystem', "rmdir failed");
}
...
bool saveConfig(fs::FS &fs, const char *path, JsonDocument &document)
{
    ...
    {
        ESP_LOGE('FileSystem', "Failed to write to file");
        file.close();
        return 0;
    }
...
void writeFile(fs::FS &fs, const char *path, const char *message)
{
    ...
    if (file.print(message))
        ESP_LOGI('FileSystem', "File written");
    else
        ESP_LOGE('FileSystem', "Write failed");
    file.close();
}
...
void appendFile(fs::FS &fs, const char *path, const char *message)
{
    ...
    if (file.print(message))
        ESP_LOGI('FileSystem', "Message appended");
    else
        ESP_LOGE('FileSystem', "Append failed");
...
void renameFile(fs::FS &fs, const char *path1, const char *path2)
{
    ...
    if (fs.rename(path1, path2))
        ESP_LOGI('FileSystem', "File renamed");
    else
        ESP_LOGE('FileSystem', "File failed");
}
...
void deleteFile(fs::FS &fs, const char *path)
{
    ...
    if (fs.remove(path))
        ESP_LOGI('FileSystem', "File deleted");
    else
        ESP_LOGE('FileSystem', "File failed");
}
...
bool initFileSystem()
{
    ...
    {
        ESP_LOGE('FileSystem', "FileSystem Mount Failed");
        return 0;
    }
    return 1;
}